...
namespace LyricsApp
{
    public partial class Form1 : Form
    {
        ...
        private async void getLyric_Click(object sender, EventArgs e)
        {
            ...
            {
                // Nếu không có dấu gạch ngang, chỉ lấy tên bài hát
                songTitle = userInput.Text;
                artist = string.Empty;
            }
            var lyrics = await GetLyrics(songTitle, artist);
            lyricShow.Text = lyrics;
        }
        ...
        private async Task<string> GetLyrics(string songTitle, string artist)
        {
            using (HttpClient client = new HttpClient()) 
            {
                ...
                string searchUrl;
                if (artist != null)
                    searchUrl = $"https://api.genius.com/search?q={Uri.EscapeDataString(songTitle + " " + artist)}";
                else
                    searchUrl = $"https://api.genius.com/search?q={Uri.EscapeDataString(songTitle)}";
                ...
                if (jsondilla["response"]["hits"].HasValues)
                {
                    ...
                    string lyrics_url = $"https://genius.com{lyrics_api}";
                    return await GetLyricsFromSongUrl(lyrics_url);
                }
                else
                    return "Không tìm thấy lời bài hát!";
            }
        ...
        private async Task<string> GetLyricsFromSongUrl(string url)
        {
            using (HttpClient client = new HttpClient())
            {
                ...
                var lyricsDivs = htmlDoc.DocumentNode.SelectNodes("//div[contains(@class, 'Lyrics__Container')]");
                if (lyricsDivs == null || lyricsDivs.Count == 0)
                    return "Không tìm thấy lời bài hát!";
                StringBuilder lyrics = new StringBuilder();

                foreach (var div in lyricsDivs)
                    foreach (var node in div.DescendantsAndSelf())
                    {
        ...
        private void _clearKeo_Click(object sender, EventArgs e)
        {
